...
const ModalAddress = () => {
    ...
    useEffect(async () => {
        ...
        const [balance118, balance750 , delegations118, delegations750, unBondings118, unBondings750] = await Promise.all(
            [
            ...
            <ReactModal.Body className="create-wallet-body import-wallet-body">
                    ...
                    <div className="d-flex mb-3">
                        <div>
                            <p className="mnemonic-result text-left pt-0 pb-2">
                                <span className='label'>{t("NEW_ADDRESS")}: </span>{response?.coin118Data?.address}
                            </p>
                            <p className="mnemonic-result text-left pt-0 pb-2">
                                <span className='label'>{t("WALLET_PATH")}: </span>{response?.coin118Data?.walletPath}</p>
                            <p className="mnemonic-result text-left pt-0 pb-2">
                                <span className='label'>{t("Coin-type")}: </span>118 </p>
                ...
                <div className="form-field radio">
                    <div className="d-flex mb-3">
                        <div>
                            <p className="mnemonic-result text-left pt-0 pb-2">
                                <span className='label'>{t("ORIGINAL_ADDRESS")} : </span>{response?.coin750Data?.address}
                            </p>
                            <p className="mnemonic-result text-left pt-0 pb-2">
                                <span className='label'>{t("WALLET_PATH")}: </span>{response?.coin750Data?.walletPath}</p>
                            <p className="mnemonic-result text-left pt-0 pb-2">
                                <span className='label'>{t("Coin-type")}: </span>750 </p>
                            <p className="mnemonic-result text-left p-0 d-flex align-items-center">
                                <span className='label m-0'>{t("Balance")}:&nbsp;</span>
                                {balanceLoading ?
                                    <Spinner
